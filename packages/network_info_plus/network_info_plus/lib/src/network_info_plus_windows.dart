...
class NetworkInfoPlusWindowsPlugin extends NetworkInfoPlatform {
  ...
  Future<String?> getWifiFrequency() async {
    return Future<String>.value(query((pGuid, pAttributes) {
      final ppWlanBssList = calloc<Pointer<WLAN_BSS_LIST>>();
      try {
        final wlanBssList = ppWlanBssList.value.ref;
        print("${wlanBssList} wlanBssList from wifi frequency");
        if (wlanBssList.dwNumberOfItems > 0) {
          print("${wlanBssList.wlanBssEntries[0]} wlanBssList from wifi frequency");
          final frequency = wlanBssList.wlanBssEntries[0].ulChCenterFrequency;
          print("${frequency} frequency from wifi frequency");
          return frequency.toString();
        } else {
          return '';
        }
      } finally {
        if (ppWlanBssList.value != nullptr) {
          WlanFreeMemory(ppWlanBssList.value);
        }
        free(ppWlanBssList);
      }
    }));
